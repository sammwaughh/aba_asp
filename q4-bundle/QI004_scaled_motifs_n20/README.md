...
QI-003's `n=100` run was computationally infeasible under the 120s Prolog timeout budget (cat3/cont3 timed out; binary hit the all-zero-positive encoding limitation; 0 cells solved before manual abort — see `../QI003_scaled_motifs/run_log.md`). QI-004 keeps the identical conceptual design but reduces `n` to 20 and raises the timeout to 300s. QI-003 is preserved unchanged as a recorded feasibility attempt.
