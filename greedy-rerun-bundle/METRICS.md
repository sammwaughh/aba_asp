...
**De-emphasis note (motif recovery).** `body_parent_f1` mixes a rule-level
precision with a variable-level recall (different denominators) and is lenient:
a rule citing a parent *and* an ancestor still counts as a full parent hit. It
is retained in the schema for cross-experiment stability, but the
motif-recovery reporting (`scripts/motif_recovery_summary.py`) does not lead
with it; prefer the variable-level metrics in §3.4b.

### 3.4b Variable-level set comparison (RQ1, stricter)

Computed by `variable_level_stats` over the **non-trivial target rules**. Let
`recovered` be the union of graph variables cited across those rule bodies,
with the target removed, and `P_t` the true parents of `t`:

| Metric | Formula | Range | Edge case |
|--------|---------|-------|-----------|
| `var_parent_precision` | \|recovered ∩ P_t\| / \|recovered\| | [0,1] | NaN (`no_recovered_vars`) if `recovered = ∅` |
| `var_parent_recall` | \|recovered ∩ P_t\| / \|P_t\| | [0,1] | NaN (`no_parents`) if `P_t = ∅` |
| `var_parent_jaccard` | \|recovered ∩ P_t\| / \|recovered ∪ P_t\| | [0,1] | NaN (`no_vars_or_parents`) if both empty |
| `clean_recovery` | `1` iff `recovered = P_t` (and `P_t ≠ ∅`), else `0` | {0,1} | `0` when unsolved / no rules |

Unlike rule-level `body_parent_precision`, `var_parent_precision` penalises
*any* ancestor/off-graph contamination in the recovered set. `clean_recovery`
