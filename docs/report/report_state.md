...
1. **Milestone 1** — report-ready account of when/how unguided ABA Learning recovers mechanism-aligned rules (and what is recovered instead). **Part 1 closed (M1.1).** **Part 2 (M1.2 / M12x) closed** (18/18 run + Stage-3). **Part 3 (M1.3) claim drafting commencing.** No M1.4. Primary path: `milestone1_high_level_path.md`.
