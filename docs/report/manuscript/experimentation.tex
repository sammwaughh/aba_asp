...
\section{QL1: initial motif-by-data-mode probe}
The first run, QL1, used nine small handcrafted cells: three motifs crossed with three data modes. The data tables had four to five rows. This run was intended as a tiny, rapid diagnostic, not as a definitive benchmark. Our goal was to test the infrastructure setup to run later tests at scale. All nine cells solved. (Table~\ref{tab:ql1-results}).

\begin{table}[htbp]
\centering
\caption{QL1 initial qualitative probe. Each cell reports the recovered base variables in learned \(x_2\)-rules and the resulting \(F_1\) score.}
\label{tab:ql1-results}
...
\section{QL2: complete truth-table baseline}
...
\begin{table}[htbp]
\centering
\caption{QL2 complete truth-table baseline. \(F_1\) measures variable-level overlap between recovered base variables and the true parent set.}
\label{tab:ql2-results}
...
\section{QL3: scaled noisy follow-up}
...
\centering
\scriptsize
\caption{QL3 data-generating mechanisms by motif and data mode. Here \(u\) denotes the true direct parent of \(x_2\) in the single-parent chain and fork cells, while \(v\) denotes the other non-target variable. All Gaussian noise variables are independent standard normal draws.}
\label{tab:ql3-dgp-mechanisms}
\begin{tabular}{lll}
\toprule
Motif & Data mode & Data-generating mechanism \\
\midrule
Chain & Binary &
\(v\sim\mathrm{Bernoulli}(0.5)\), \(u=\mathrm{flip}_{0.2}(v)\), \(x_2=\mathrm{flip}_{0.1}(u)\). \\
Chain & Categorical-3 &
\(v\sim\mathrm{Uniform}\{0,1,2\}\), \(u=\mathrm{catnoise}_{0.2}(v)\), \(x_2=\mathrm{catnoise}_{0.1}(u)\). \\
Chain & Continuous &
\(v\sim\mathcal{N}(0,1)\), \(u=v+0.5\varepsilon_u\), \(x_2=u+0.3\varepsilon_t\). \\
\midrule
Fork & Binary &
\(u\sim\mathrm{Bernoulli}(0.5)\), \(v=\mathrm{flip}_{0.2}(u)\), \(x_2=\mathrm{flip}_{0.1}(u)\). \\
Fork & Categorical-3 &
\(u\sim\mathrm{Uniform}\{0,1,2\}\), \(v=\mathrm{catnoise}_{0.2}(u)\), \(x_2=\mathrm{catnoise}_{0.1}(u)\). \\
Fork & Continuous &
\(u\sim\mathcal{N}(0,1)\), \(v=u+0.6\varepsilon_v\), \(x_2=u+0.3\varepsilon_t\). \\
\midrule
Collider & Binary &
\(x_0,x_1\sim\mathrm{Bernoulli}(0.5)\) i.i.d., \(b=x_0\lor x_1\), \(x_2=\mathrm{flip}_{0.1}(b)\). \\
Collider & Categorical-3 &
\(x_0,x_1\sim\mathrm{Uniform}\{0,1,2\}\) i.i.d., \(b=\max(x_0,x_1)\), \(x_2=\mathrm{catnoise}_{0.1}(b)\). \\
Collider & Continuous &
\(x_0,x_1\sim\mathcal{N}(0,1)\) i.i.d., \(x_2=x_0+x_1+0.3\varepsilon_t\). \\
\bottomrule
\end{tabular}
\end{table}
...
All fifteen cells terminated without timeout. The outcome distribution was one solved cell, twelve completed no-solution cells, and two early error cells. No cell achieved exact parent-set recovery, and the maximum variable-level \(F_1\) score was \(0.67\). Table~\ref{tab:ql3-results} summarises the QL3 outcomes at the level of recovered base variables.

\begin{table}[htbp]
\centering
\scriptsize
\caption{QL3 scaled noisy follow-up at \(n=20\). \(F_1\) measures variable-level overlap between recovered base variables and the true parent set.}
\label{tab:ql3-results}
\begin{tabular}{llllcccl}
\toprule
