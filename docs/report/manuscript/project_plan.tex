...
\begin{quote}
\textbf{Live-status note (8 August 2026).} This chapter is an earlier project-plan draft,
not the current Milestone~2 approach. Its bridge architecture, staged milestones, outputs,
...
\texttt{docs/research/milestone\_plans/milestone2/README.md}.
\end{quote}

\section{Purpose and Strategy}
\label{sec:project-plan-purpose}

The remaining project investigates how ABA Learning and Causal ABA can be combined to learn causal relationships from data in an argumentative form. The experimentation so far provides a useful starting point: the current \texttt{aba\_asp/causal} pipeline can learn target-wise ABA rules from controlled synthetic data, and the variables appearing in learnt rule bodies can be compared with known parent sets. However, those experiments only measure a proxy for causality. Learnt rules may recover a direct parent, but they may also recover an ancestor, a correlated proxy, a partial parent set, or no usable rule at all.
...
\label{sec:project-plan-rqs}

\textbf{RQ1} asks when unguided ABA Learning recovers direct-parent-like variables in learnt target rules, rather than ancestors, descendants, siblings, correlated non-parents or representation artefacts. \textbf{RQ2} asks whether Causal ABA-style evidence and graph constraints can guide ABA Learning. \textbf{RQ3} asks how the resulting causally guided ABA Learning bridge compares with representative state-of-the-art causal-discovery methods on canonical examples, and whether the comparison reveals a publishable contribution in interpretability, robustness, causal alignment or argumentative explanation.
...
\section{Milestones}
\label{sec:project-plan-milestones}

\subsection{Milestone 1: Parent-Set Diagnostic, Weeks 1--2}
\label{subsec:milestone-parent-set}
...
\subsection{Milestone 2: Bridge Between ABA Learning and Causal ABA, Weeks 3--4}
\label{subsec:milestone-bridge}
...
\subsection{Milestone 3: Causal ABA-Guided ABA Learning, Weeks 5--6}
\label{subsec:milestone-guided-learning}
...
\subsection{Milestone 4: Scaling and Comparator Setup, Weeks 7--8}
\label{subsec:milestone-scaling-guided-learning}
...
\subsection{Milestone 5: Evaluation and Full Draft, Weeks 9--10}
\label{subsec:milestone-evaluation}
...
\subsection{Milestone 6: Finalisation and Presentation, Weeks 12--13}
\label{subsec:milestone-finalisation}
...
\section{Schedule}
\label{sec:project-plan-schedule}
\begin{table}[htbp]
\centering
\scriptsize
\caption{Fourteen-calendar-week schedule. Week~11 is the 24--31 August family holiday.}
\label{tab:project-plan-schedule}
\begin{tabular}{p{0.08\textwidth}p{0.16\textwidth}p{0.24\textwidth}p{0.44\textwidth}}
\toprule
...
\section{Success Criteria and Risks}
\label{sec:project-plan-success-risks}
