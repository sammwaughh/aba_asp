...
| M11-C-003 | On cat3 A vs B (and analogously D vs C), nd folding’s first literal tried follows BK predicate-block order; **non-parent-first** (isolated variable first in BK) yields a fold that does not entail E+/E− and triggers assumptions; parent-first yields entailment and a clean singleton. | bounded | `cells/m11_cat3_A/prolog.stdout`; `cells/m11_cat3_B/prolog.stdout`; bk.aba order | Mechanistic reading from trace; “non-parent” is the isolated factorial variable in the tabular DGP, not a chain ancestor. Does not prove all cat3 failures share this path without further cells. | Experimentation / Progress; findings tex | active |
