...
| M11-C-003 | On cat3 A vs B (and analogously D vs C), nd folding’s first literal tried follows BK predicate-block order; **non-parent-first** (isolated variable first in BK) coincides with a fold that does not entail E+/E− and triggers assumptions; parent-first coincides with entailment and a clean singleton. | bounded (provisional) | `cells/m11_cat3_A/prolog.stdout`; `cells/m11_cat3_B/prolog.stdout`; bk.aba order | **Correlation only — insufficient for m1.1 closure** (Fabrizio, June 2026). Does not state why entailment fails/succeeds, when each step fires, or how the runner orchestrates the path. Full mechanistic account outstanding. | Experimentation / Progress; findings tex | active |
| M11-C-004 | Greedy folding may reduce cat3 representation-order sensitivity on the m1.1 grid. | tentative → **weakened** | QI-002 greedy precedent; m1.1 cat3 σ failure | **Not established.** Greedy smoke test on m1.1 returned outcome materially different from **both** nd cat3 A and nd B. ND vs greedy is **not** sufficient to explain σ failure. M1.2 full grid deferred pending m1.1 trace investigation. | Project Plan / M1.2 | pending |
| M11-C-005 | m1.1 initial eight-cell nd grid: binary passes all σ and π checks; cat3 fails σ-invariance (A↔B, D↔C) while π holds on B↔C only. | bounded | M1.1 record; transformation table; sol files | Metamorphic verdicts from first run only; mechanistic explanation of cat3 divergence not closed. | Experimentation / Progress | active |
