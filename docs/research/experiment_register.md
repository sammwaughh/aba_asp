...
- Status: run — Stages 0–2 complete (2026-07-08). Stage 0: fixtures validated (Prolog-free checks PASS), expected outputs locked. Stage 1: config-consulting runner + feature-BK construction (+ RuleML `domain/1`) + three arm YAMLs + summary side-car (Prolog-free tests PASS), smoke-tested. Stage 2: 15-cell grid run (3 arms × 5 fixtures), 15/15 `solved`, each shipped config applied verbatim; outcome matrix `M12_summary.md`. Raw finding (not interpreted): every cell now learns positive target rules; 1 exact match (ECAI m12_fork `x2(A):-x0_val_2(A)`); AAMAS assumption-free positive rules (covers+rejects both Y); ECAI assumption-guarded rules isolating the parent; RuleML assumption-guarded full-conjunction rules incl. the isolated x2 on conj/disj; ECAI m12_chain cites the ancestor x0. Stages 3–4 not started.
