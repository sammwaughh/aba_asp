...
- Status: run — Stages 0–2 complete (2026-07-08). Stage 0: fixtures validated (Prolog-free checks PASS), expected outputs locked. Stage 1: config-consulting runner + shared default-assumption BK + three arm YAMLs + summary side-car (103 Prolog-free tests PASS), smoke-tested. Stage 2: 15-cell grid run (3 arms × 5 fixtures) — 15/15 `solved`, 0 timeout/error; each shipped config applied verbatim + distinct per arm; outcome matrix `M12_summary.md`. Raw finding (not interpreted): no cell exactly matches the locked expected rule; learned δ is assumption/contrary structure over the default rule; RuleML δ far larger than AAMAS (e.g. conj 25 vs 1 assumption/contrary). Stages 3–4 not started.
...
- Stage-1 artefacts: causal/run_aba_asp.py (`prolog_config` + `_build_learning_program`); causal/experiments/run_grid.py (thread `prolog_config`/`default_assumption`); causal/argcausaldisco_integration.py (`default_assumption_target` BK block); causal/configs/experiments/M12_{ecai2024,ruleml2025,aamas2025}.yaml; causal/experiments/m12_summary.py (side-car); tests test_m12_configs.py / test_m12_summary.py / test_runner_program.py + extended test_runner_stage2.py.
- Stage-2 artefacts: causal/outputs/aba_learning/grid/M12_{ecai2024,ruleml2025,aamas2025}/ (cells/<fixture>/ via `grid.cell_dir: dgp`; results.parquet + manifest.json + run.log); causal/outputs/aba_learning/grid/M12_summary.md (+ .json). Env: SWI-Prolog 10.0.2, Python 3.10.20, commit 9bd8e322. Post-Stage-2: hash cell dirs renamed to fixture ids; grids re-run once after YAML `cell_dir: dgp` (config_hash bump); resume verified.
- Fork design note: m12_fork uses Samuel's deterministic two-child fork (x1 := 2 if x0=1 else 0; x2 := 2 if x0=2 else 0; three assignments ×3 = 9 rows), replacing the unsatisfiable "complete factorial + associated sibling" draft; x1's alphabet is {0,2} to stay on the value-predicate encoding path.
...
- Code path: causal/experiments/handcrafted_m12.py (fixtures); runner consults a `prolog_config` .pl file verbatim (done, Stage 1); shared default-assumption BK (done); M12 metric side-car causal/experiments/m12_summary.py (done); timeout 60 s.
- Dataset / DGP: five handcrafted categorical fixture families (k=3; p fixture-dependent), each declaring its graph G in code, config, and reporting: separator anchor, conjunctive mechanism on a collider (p=3, x3 := min(x0,x1), isolated x2), disjunctive mechanism on a collider (symmetric twin of conj: p=3, x3 := max(x0,x1), isolated x2), fork (correlated-sibling confound, deterministic two-child design; different table from the QI-002 factorial fork), correlated-ancestor chain (cyclic-deviation blocks; P(pos|x0)=0, 1/3, 2/3).
