...
- Dataset / DGP: five handcrafted categorical fixture families (k=3; p fixture-dependent), each declaring its graph G in code, config, and reporting: separator anchor, conjunctive mechanism on a collider (p=3), disjunctive mechanism on a collider, fork (correlated-sibling confound; extends QI-002 fork baseline), correlated-ancestor chain.
