...
| M12 | — | m1.2 Published-configuration comparison (ASP-ABAlearnB / RASP-ABAlearn / Greedy ABA Learning) | planned (Stage 0 complete: fixtures + checks + locked expected outputs) |
...
- Status: planned — Stage 0 complete (2026-07-08): fixtures implemented and validated (59 Prolog-free checks PASS), expected outputs locked in the record before any learning run. Stages 1–4 not started.
- Planning doc: docs/research/milestone_plans/milestone1_part2/milestone1_part2_config_comparison.md.
- Record: docs/experiments/qualitative/M1.2-config-comparison.md (Stage-0 sections filled: exact tables, E+/E-, locked expected outputs, validation commands/results).
...
- Code path: causal/experiments/handcrafted_m12.py (fixtures, done); planned: runner extended to consult a `prolog_config` .pl file; adapted metrics/outcome classifier; timeout 60 s.
