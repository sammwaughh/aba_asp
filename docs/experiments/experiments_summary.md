...
| M13 | — | m1.3 causal-recovery capabilities and limits of unguided ABA Learning | in progress | `M1.3-bucket1-claims.md` (locked); `M1.3-bucket2-claims.md` (locked); `M1.3-bucket3-claims.md` (direction approved; first investigation pending) | — | Deterministic fixture/learning artefacts: none yet. Preserved stochastic diamond artefacts are pre-pivot only. | Interim Experimentation / Progress (Milestone 1) | Buckets 1–2 remain locked. Following Fabrizio's 31 July guidance, Bucket 3 will analyse one causally sufficient, exactly certified root-stochastic/non-root-deterministic fixture at a time, with evaluator-only mechanism rules stated before ABALearn. |
...
Claims 1–2 from M13-C1/C2; Bucket 3 has an approved deterministic-mechanism direction,
but no first deterministic research fixture, run, or claim. M12x and Buckets 1–2 remain
fixed evidence. Method:
...
The immediate method is to define one DAG with non-degenerate stochastic roots and
deterministic non-root truth tables; certify its exact support, ordinary faithfulness,
and standard MEC/CPDAG; record evaluator-only mechanism rules; then approve a frozen
sample and run every target before inspecting the learned frameworks. The existing
positive-stochastic diamond and its AAMAS/ECAI target-wise collections are preserved
pre-pivot exploratory/infrastructure artefacts, not evidence for this direction.
...
- Which first deterministic mechanism supplies a clear parent-aligned rule reference
  while retaining a fixture-specific ordinary-faithfulness certificate?
- For each target, which rule branches are formally defined, population-supported, and
  present in the finite sample, and how do ABALearn's learned rules relate to them?
- Which observations are attributable to learner strategy rather than graph
  identifiability, mechanism representation, support, or finite-sample availability?
...
- Missingness, larger graphs, parameter/category variation, and non-discrete data remain
  deferred until the first deterministic case has been inspected and written up.
...
**Choose and approve the first deterministic Bucket 3 fixture**. Define its graph,
root distributions, deterministic truth tables, exact support/faithfulness certificate,
and evaluator-only rule reference before sampling or running ABALearn. Do not create a
claim or run matrix in advance.
