...
| M13 | — | m1.3 causal-recovery capabilities and limits of unguided ABA Learning | in progress | `M1.3-bucket1-claims.md` (locked); `M1.3-bucket2-claims.md` (locked); `M1.3-bucket3-claims.md` (M13-C3 H0 closed; H1–H2 analysed; no claim) | — | M13-C3 **H0 closed**; **H1–H2 run/analysed**; H3–H4 deferred. | Interim Experimentation / Progress (Milestone 1) | Bucket 3: H0 closed; H1–H2 analysed; next = H3–H4; no claim yet. |
...
| M13-C3 | M13 | Bucket 3 binary deterministic AND collider | **H0 closed**; **H1–H2 analysed** (no claim) | `experiment.md`; `learning_analysis.*`; `h1_support_ablation.*`; `h2_irrelevant_covariate.*`; `future_probes.*` | H0/H1 aamas configs; H2 `.../m13_bucket3_binary_collider_and_iso_d/aamas2025/{n30,n2}_seed42.yaml` | H0/H1 collections + H2 iso_d AAMAS n30/n2 | Interim Experimentation / Progress (Milestone 1) | H2 lead: AAMAS `c` retains irrelevant `d_val_*` (not D-free AND). H1: roots incorrectly solved on n25. No Bucket 3 claim. |
...
**H1–H2 run / analysed** (**no claim**). Next: deferred probes H3–H4.
...
Bucket 3 (M13-C3 **H0 closed**; **H1–H2 analysed**; no claim):
...
- How should deferred probes H3–H4 be sequenced after closed H0 and analysed
  H1–H2?
...
**Open deferred M13-C3 probes H3–H4 when ready** (`future_probes.md`).
