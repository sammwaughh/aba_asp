...
H2 and H3 are documented in \texttt{h2\_irrelevant\_covariate.md} and
\texttt{h3\_bk\_leading\_distractor.md}; H4 and H4b are run / analysed
(\texttt{h4\_cautious\_vs\_brave.md},
\texttt{h4b\_cautious\_split\_under\_a.md}); H5 and H6 analysed
(\texttt{h5\_greedy\_cautious.md}, \texttt{h6\_folding\_and\_n\_ablation.md});
...
\begin{itemize}
  ...
  \item Not a claim that omitting \((0,0,0)\) is the only way AAMAS can emit a
    spurious root rule.
  \item H3--H4 and H4b are untouched by this record (H2 is a separate later probe).
\end{itemize}
...
Probes H2 and H3 are documented in \texttt{h2\_irrelevant\_covariate.md} and
\texttt{h3\_bk\_leading\_distractor.md}.
H4, H4b, H5, and H6 are run / analysed as recorded in
\texttt{future\_probes.md}; H7a is analysed
(\texttt{h7a\_relto\_vs\_sechk.md}); H7b is analysed separately
(\texttt{h7b\_cautious\_relto\_vs\_sechk.md}).
