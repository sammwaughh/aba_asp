...
\large M1.3 Bucket 3 synthesis of H0--H7a}
...
This is a supervisor-facing working synthesis of the completed H0--H7a
evidence. It is not submitted-report prose and it does not lock a Bucket~3
claim. H7b, the cautious-nd \code{relto}--\code{sechk} comparison, is now run
and analysed but is not yet integrated into the findings and candidate claims
below.
...
Across H0--H7a, \ABALearn{} sometimes emits the direct AND rule, but this
...
predictors. The experiments therefore provide a controlled account of
...
\end{lstlisting}
  ...
  \item \textbf{Assumption introduction changes retained variables.}
  Under brave nd, changing \code{relto} to \code{sechk} changes all seven
  paired deltas. Here \code{sechk} latches onto the first BK predictor and
  closes with an assumption chain, while \code{relto} more often reaches later
  covariates after failed reuse.
\end{enumerate}
...
\code{greedy_cautious} and \code{nd_brave_sechk} are experimental, not
published AAMAS or ECAI methods. H6 configurations match
...
\Hprobe{7b} & Cautious counterpart to H7a & Underway &
\textbf{Excluded from this evidence.} \\
\bottomrule
\end{longtable}
...
H4, H4b, H5, H6a, and H7a hold fixture, sample, encoding, and relevant inputs
...
\section{Assumption introduction under brave nd: H7a}

\subsection{Design and roles}
...
\subsection{Trace-supported explanation}
...
recovery. H7b is documented separately and intentionally absent from this
H0--H7a synthesis version.
...
\begin{itemize}
  ...
  \item brave-nd \code{sechk} drops later mechanism parents in H7a; and
...
Assumption policy & H7a & \code{relto}/\code{sechk} take different repair paths
and retain different covariates. \\
...
\begin{itemize}
  ...
  \item H7a: assumption-introduction control flow.
\end{itemize}
...
frameworks. These are motivations, not H0--H7a results.
...
revisited after H7b is integrated and after supervisor review.
...
with isolated variables; H7a solves all cells with different deltas.
...
has 18/18 outcome matches and byte-identical solved deltas.
...
\textbf{Statement.} Under brave nd on H0/H3 \(n=30\), switching
\code{relto} to \code{sechk} changes every delta; \code{sechk} follows a
first-BK chain while \code{relto} more often reaches later covariates.

\evidence Seven exact input-matched H7a pairs; all outcomes solved, all deltas
different, all inspected traces share the initial fork.

\boundary Bounded to brave nd on two fixtures. H7b now supplies the cautious
counterpart but must be integrated before any cross-semantic extension. No
method ranking.
...
\begin{itemize}
  ...
  \item H7b is run / analysed but not yet integrated into this H0--H7a synthesis.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Which candidate claims need another fixture or seed?
\end{enumerate}

\section{Conclusion}

H0--H7a provide a controlled, trace-backed account of target-wise ABA Learning
...
H7a assumption-introduction control flow. This attribution is the principal
...
Bucket~3 hub still records no approved claim. H7b must be integrated before
the H7 story is closed. Later report wording must be checked against canonical
experiment records and the claims ledger.
