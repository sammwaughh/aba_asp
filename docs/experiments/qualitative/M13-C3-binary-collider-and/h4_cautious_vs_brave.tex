...
\begin{itemize}
  ...
  \item H5, H6, and H7a analysed separately
    (\texttt{h5\_greedy\_cautious.md},
    \texttt{h6\_folding\_and\_n\_ablation.md},
    \texttt{h7a\_relto\_vs\_sechk.md}); H7b analysed separately
    (\texttt{h7b\_cautious\_relto\_vs\_sechk.md}).
    H6/H7a/H7b are not this H4 evidence body.
\end{itemize}
...
H7a (brave nd \texttt{asm\_intro(sechk)} vs \texttt{relto}) were prompted by
H4 traces and are now run / analysed in separate records. H7b is also run /
analysed separately (\texttt{h7b\_cautious\_relto\_vs\_sechk.md}).

\section{Next}

H4 documentation is complete.
H4b and H5 are run / analysed separately.
H6, H7a, and H7b are analysed separately.
