...
\author{}
\date{Pre-run stage (ABALearn not yet run)}

\begin{document}
\maketitle

\noindent\textbf{Status.} Active M1.3 Bucket~3 investigation; fixture certified;
sample \(n{=}30\), seed~\texttt{42} frozen; \textbf{no ABALearn run}; \textbf{no Bucket~3 claim}.
Companion evidence record:
\texttt{docs/experiments/qualitative/M13-C3-binary-collider-and/experiment.md}.
This file is a working mathematical dossier, not report-facing claim prose.
...
\begin{enumerate}
  ...
  \item \emph{Future} table-derived learner inputs (BK, \(E^+\), \(E^-\)).
  \item \emph{Future} ABALearn outputs (delta / final framework).
  \item Evaluator-only mechanism reference and bounded interpretation.
\end{enumerate}
Items 1--5 and 8 are recorded here. Items 6--7 do not yet exist for this fixture.
...
\section{Implied target counts under the planned binary policy}

Planned (not yet executed) target-wise policy: target value \(1\) as \(E^+\),
value \(0\) as \(E^-\). Counts \emph{implied} by the frozen sample marginals:
...
\section{Open next step}

AAMAS target-wise learning on the frozen sample has since been executed
(see the investigation record and collection summary under
\texttt{causal/outputs/aba\_learning/targetwise/m13\_bucket3\_binary\_collider\_and/aamas2025/n30\_seed42/}).
Near-term controlled follow-ups are recorded in
\texttt{future\_probes.tex} / \texttt{future\_probes.md}:
H1--H3 designs were \textbf{approved for investigation} on 2026-08-01;
H4 (cautious vs brave on ECAI roots) is \textbf{proposed} and
\textbf{blocked} on target-wise cautious support.
All remain \textbf{not tested} and \textbf{not claims}.
No Bucket~3 claim follows from the baselines or from those signposts alone.
