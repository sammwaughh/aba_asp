# QI-004 metrics (QI004_scaled_motifs_n20)
...
This file records which metrics QI-004 leads with and what each one measures. Full definitions live in `causal/aa-plans/METRICS.md`; the variable-level metrics are §3.4b. Metric definitions are identical to QI-003.
...
QI-004's design goal is to break the QI-001 x0/first-column confound via parent-position variants. The decision-critical reading is the `clean_recovery` (and `var_parent_*`) comparison between the `*_x0parent` and `*_x1parent` variants of each motif:
...
Continuous cells (`*_cont3`) are binned into 3 uniform bins at BK-generation time (`bins=3`, `bin_strategy=uniform`). With `n=20` the bins are smaller than QI-003's `n=100`; a pre-run check (approximate uniform binning) found all 3 bins non-empty for `x0`/`x1` across the continuous fixtures (minimum bin count 2). Bin-health is recorded per cell in the raw status table, since sparse/empty bins would make the categorical predicate vocabulary degenerate.
