...
> Scope: QI-004 evaluates **target-wise parent-set recovery** under the **current `aba_asp/causal` implementation** (the **target-wise ABA Learning pipeline**). It does NOT evaluate full Russo-style Causal ABA, graph recovery, d-separation reasoning, `arr`/`noe`/`indep`, or stable-extension-as-DAG machinery. "Recovery" throughout means alignment between **learned target-rule bodies** for `x2` and the known direct parents of `x2`; it is not causal discovery.
...
QI-004 (`QI004_scaled_motifs_n20`) was introduced after the QI-003 (`QI003_scaled_motifs`, `n=100`) run proved computationally infeasible: under a 120 s Prolog timeout its categorical-3 / continuous-3 cells timed out and its binary cells failed instantly on an encoding limitation, so QI-003 produced **0 solved cells** and no parent-recovery evidence (it is preserved only as a feasibility/limitation note).

QI-004 keeps the QI-003 design idea — **scaled, noisy motifs with parent-position controls**, across binary / categorical-3 / continuous-binned data modes — but changes only the two computational-feasibility settings:
...
- **Fork variants (`x0parent`, `x1parent`).** No fork cell recovered anything: four returned `completed_no_solution` and `fork_x0parent_binary` errored on the binary encoding limitation. This contrasts with QI-001/QI-002, where fork (under canonical orientation, parent `x0`) recovered cleanly across modes.
...
- **Parent position vs x0 preference.** The intended confound check (does recovery track the true-parent column or favour `x0`?) **cannot be answered by QI-004**, because only one cell solved and it is a `parent_superset` citing *both* columns. There is no cell where the pipeline recovered exactly `{x1}` or exactly `{x0}`, so "tracks the true parent" vs "favours x0" remains undetermined. The parent-position **fixtures** are correctly constructed (verified: the true parent sits in the asserted column for every cell), but the run did not yield enough solved cells to exercise the comparison.
...
- **Binary.** 5 cells: 1 solved (superset), 2 `completed_no_solution`, 2 `error` (the all-zero-positive `unknown constant` limitation, in `fork_x0parent_binary` and `collider_binary`). Binary is the only mode that produced any rule, consistent with QI-001/QI-002 where binary was the friendliest encoding — but here even binary does not produce a clean recovery.
- **Categorical-3.** 5 cells, all `completed_no_solution`; no rules, no recovery.
- **Continuous-binned (3 uniform bins).** 5 cells, all `completed_no_solution`; no rules. Crucially, **all continuous cells now complete** (no timeouts), which is the concrete improvement over QI-003, where every cat3/cont3 cell timed out. So QI-004 changes the continuous picture from "cannot be evaluated (timeout)" to "evaluated, and recovers nothing at `n=20`".
- **Binning / sample size.** Bin occupancy was healthy (Section 2), so the continuous non-recovery is not a binning-degeneracy artefact. The most visible remaining factor is sample size: at `n=20` with mild noise the learner overwhelmingly returns `completed_no_solution` rather than a parent rule. Whether this is driven by `n`, by noise, or by the learner/encoding interaction is not separable from QI-004 alone.
...
- **Does QI-004 rescue the continuous analysis after QI-003 timed out?** Partially. It removes the *feasibility* blocker — all continuous (and categorical) cells now complete within budget — so the continuous question is finally answerable. The answer at `n=20`, however, is that the pipeline recovers nothing in continuous mode (all `completed_no_solution`).
- **Does QI-004 show the implementation can scale modestly?** Yes, in the narrow sense that `n=20` with a 300 s timeout completes 15/15 cells without timeouts (vs `n=100` timing out at 120 s). This is a feasibility result, not a recovery result.
- **Does QI-004 show parent-position controls matter?** Not yet — the controls are correctly built, but only one cell solved, so the `*_x0parent` vs `*_x1parent` comparison could not be exercised. The confound remains empirically unbroken.
- **Does QI-004 become the canonical scaled/noisy qualitative evidence?** Yes — QI-004 is the canonical *scaled, noisy* run (it completes and is interpretable), superseding QI-003 for that role. But its substantive recovery finding is essentially null (0/15 clean), so the canonical *recovery* evidence remains QI-001 (encoding/motif sensitivity) and QI-002 (best-case learnability, clean_recovery 0.5).
- **Does any QI-003 evidence remain useful?** Only as an audit/feasibility note: QI-003 documents the `n=100` computational wall and the binary encoding limitation that motivated QI-004. It contributes no recovery evidence.
...
| Reducing `n` from 100 to 20 (and raising the timeout) resolves the QI-003 categorical/continuous timeout wall | Yes | QI-003: all cat3/cont3 cells timed out; QI-004: all cat3/cont3 cells complete | Recovery in those completed cells is null |
...
| QI-004 is the canonical *scaled/noisy* qualitative run (supersedes QI-003 for that role) | Yes | QI-004 completes and is interpretable; QI-003 is a feasibility attempt | Its recovery finding is null; canonical *recovery* evidence stays QI-001/QI-002 |

### Claims NOT supported

| Claim not supported | Reason |
|---|---|
| QI-004 breaks the x0/first-column confound | Only 1 cell solved (a superset citing both columns); the `*_x0parent` vs `*_x1parent` comparison was not exercised |
| The pipeline recovers parents at scale under noise | 0/15 clean recovery at `n=20` |
| Continuous/categorical recovery improved over QI-001 | QI-004 continuous/categorical cells recover nothing (all `completed_no_solution`); QI-001 at least produced rules (though imperfect) |
| QI-004 demonstrates causal discovery / DAG recovery / edge orientation | Out of scope; target-wise parent-set recovery only; reserved graph metrics not computed |
