...
| M12 | — | m1.2 Published-configuration comparison (ASP-ABAlearnB / RASP-ABAlearn / Greedy ABA Learning) | planned (Stages 0–1 complete: fixtures + checks + locked expected outputs; runner/BK/arm-configs/summary + smoke test) |
...
- Status: planned — Stages 0–1 complete (2026-07-08). Stage 0: fixtures implemented and validated (Prolog-free checks PASS), expected outputs locked in the record before any learning run. Stage 1: config-consulting runner + shared default-assumption BK + three arm YAMLs + summary side-car built and unit-tested (103 Prolog-free tests PASS), smoke test confirms effective options match the config file. Stages 2–4 not started.
