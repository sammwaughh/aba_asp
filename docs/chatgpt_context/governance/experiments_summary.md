...
| M12 | — | m1.2 Published-configuration comparison (ASP-ABAlearnB / RASP-ABAlearn / Greedy ABA Learning) | run (Stages 0–2 complete; Stage 3 next) | `docs/experiments/qualitative/M1.2-config-comparison.md` | `causal/configs/experiments/M12_ecai2024.yaml`; `M12_ruleml2025.yaml`; `M12_aamas2025.yaml` (`grid.cell_dir: dgp`) | `causal/outputs/aba_learning/grid/M12_{ecai2024,ruleml2025,aamas2025}/cells/<fixture>/`; `M12_summary.md` | Interim Experimentation / Progress (Milestone 1) | Stages 0–1 (2026-07-08): fixtures + checks + locked expected outputs; config-consulting runner, default-assumption BK, arm YAMLs, summary side-car (103 Prolog-free tests PASS). Stage 2 (2026-07-08): 15-cell grid run — **15/15 solved**, 0 timeout/error; each shipped config applied verbatim + distinct per arm (ECAI nd/any/all; RuleML greedy/mgr/bk + entailment false; AAMAS greedy/mgr/bk + entailment true). Post-Stage-2: cell dirs renamed to fixture ids (`m12_sep`, …). No cell is an exact match to the locked expected rule; learned δ is assumption/contrary structure over the default rule (raw evidence; interpretation is Stage 3). Incoherent-table gate probe deferred. |
...
expected outputs; config-consulting runner, default-assumption BK, three arm YAMLs, summary
side-car; 15-cell grid run, all solved, outcome matrix built). Next: Stage 3 — per-cell
expected-vs-learned comparison and categorical outcome-class labelling. See
