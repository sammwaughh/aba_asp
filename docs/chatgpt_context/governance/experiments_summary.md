...
| M12 | — | m1.2 Published-configuration comparison (ASP-ABAlearnB / RASP-ABAlearn / Greedy ABA Learning) | planned (Stages 0–1 complete) | `docs/experiments/qualitative/M1.2-config-comparison.md` | `causal/configs/experiments/M12_ecai2024.yaml`; `M12_ruleml2025.yaml`; `M12_aamas2025.yaml` | `causal/outputs/aba_learning/grid/M12_<arm>/` (Stage 2) | Interim Experimentation / Progress (Milestone 1) | Grid not yet run. Stage 0 (2026-07-08): fixtures (sep/conj/disj/fork/chain), Prolog-free checks PASS, expected outputs locked. Stage 1 (2026-07-08): config-consulting runner (`prolog_config`), shared default-assumption BK, three arm YAMLs, summary side-car `causal/experiments/m12_summary.py`; 103 Prolog-free tests PASS; smoke test (M12_ecai2024/m12_sep) confirms effective `listing(lopt/1)` matches the config file. Plan: 3 configs × 5 fixtures (15 cells). Incoherent-table gate probe deferred. |
...
**Milestone 1 Part 2 (M1.2)** — Stages 0–1 complete (fixtures + validation checks + locked
expected outputs; config-consulting runner, default-assumption BK, three arm YAMLs, and
summary side-car, smoke-tested). Next: Stage 2 — run the 3 arms × 5 fixtures = 15-cell
grid and build the outcome matrix. See
