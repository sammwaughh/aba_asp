"""Prolog-free tests for QI-004 reduced-n scaled fixtures and config.

QI-004 mirrors QI-003's design at n=20 (computationally feasible follow-up).
Validate n=20 generation, parent-position controls (the true parent sits in the
...
from causal.experiments.handcrafted_qi004 import QI004_ALIASES, QI004_BUILDERS
from causal.experiments.run_grid import execute_cell_stage2
from aba_asp.utils.data_utils import bin_continuous_variable

_N = 20

_CHAIN_IDS = ("qi004_chain_x1parent", "qi004_chain_x0parent")
_FORK_IDS = ("qi004_fork_x0parent", "qi004_fork_x1parent")
_COLLIDER_IDS = ("qi004_collider",)
...
_CONFIG_PATH = _REPO_ROOT / "causal/configs/experiments/QI004_scaled_motifs_n20.yaml"
...
def _cell_for(source: str) -> CellSpec:
    ...
    return CellSpec(
        experiment_id="QI004_scaled_motifs_n20",
        dgp=source,
        nodes=fx.nodes,
        edges=fx.edges,
        n=len(fx.df),
...
def test_module_registers_fifteen_fixtures() -> None:
    assert len(QI004_BUILDERS) == 15
    assert set(QI004_BUILDERS) == set(_ALL_IDS)
    assert set(QI004_ALIASES) == set(_ALL_IDS)
...
@pytest.mark.parametrize("source", _ALL_IDS)
def test_twenty_rows_nodes_target(source: str) -> None:
    fx = load_handcrafted(source)
    assert len(fx.df) == _N
    assert fx.nodes == 3
...
def test_continuous_bins_populated(source: str) -> None:
    ...
    for col in ("x0", "x1"):
        ...
        # n=20 with 3 uniform bins: a tail bin can be sparse but should be
        # non-empty for these fixed seeds. If a future seed empties a tail bin,
        # relax to ">= 2 of 3 bins non-empty"; for now assert all populated.
        for b in (0, 1, 2):
...
def test_config_uses_extended_timeout() -> None:
    cfg = load_config(_CONFIG_PATH)
    assert float(cfg.defaults["prolog_timeout_s"]) == 300.0
    assert cfg.experiment_id == "QI004_scaled_motifs_n20"
