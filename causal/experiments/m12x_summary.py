...
def _is_val_expansion_of_nz(learned_norm: set[str], hstar_norm: frozenset[str]) -> bool:
    """True if learned is a pure val_1/val_2 expansion of a single-literal nz H*."""
    # Only handle the common case: H* = {t(A):-xi_nz(A)} and learned =
    # {t(A):-xi_val_1(A), t(A):-xi_val_2(A)}.
    if len(hstar_norm) != 1:
        return False
    h = next(iter(hstar_norm))
    if ":-" not in h:
        return False
    head, body = h.split(":-", 1)
    if not body.endswith("_nz(A)") or "," in body:
        return False
    base = body[: -len("_nz(A)")]
    expected = {
        f"{head}:-{base}_val_1(A)",
        f"{head}:-{base}_val_2(A)",
    }
    return learned_norm == expected
...
def _flags_and_judgment(
    *,
    ...
    if val_exp:
        flags.append("val_expansion_of_hstar")
    ...
    if exact:
        sketch = "exact match to compact nz H*"
    elif val_exp:
        sketch = "val-expansion of H* (inferior but related)"
        ...
        sketch = "assumption-mediated; not compact nz H*"
    else:
...
def summarize_cell(cell_dir: Path, arm: str) -> M12xCellSummary | None:
    ...
    val_exp = (not exact) and _is_val_expansion_of_nz(learned_norm, hstar)
    ...
    return M12xCellSummary(
        ...
        exact_hstar=exact,
        val_expansion_of_hstar=val_exp,
...
def build_markdown(rows_by_arm: dict[str, list[M12xCellSummary]]) -> str:
    lines: list[str] = [
        ...
        "",
        "Flag legend: `exact_hstar` · `val_expansion_of_hstar` · `cites_descendant` · "
    ...
    for arm in ARMS:
        ...
        n_val = sum(1 for r in rows if r.val_expansion_of_hstar)
        ...
        lines.append(
            ...
            f"val-expansion: {n_val}; cites descendant: {n_desc}; "
            f"cites other distractor: {n_dist}."
        )
        lines.append("")
        lines.append(
            "| fixture | target | outcome | exact | val-exp | desc | distractor | "
        ...
        for r in rows:
            lines.append(
                ...
                f"{'Y' if r.val_expansion_of_hstar else 'N'} | "
