...
Regime: alphabet {0,1,2}; deterministic copy/min/max; full source factorial;
nonzero-positive labels (E+ = t != 0); definitional ``*_nz`` BK is enabled by the
M12x experiment YAML (``defaults.definitional_nz: true``), not in this module.
...
def m12_u5_chain_double_copy() -> HandcraftedFixture:
    """U5: chain; x1 := copy(x0), x2 := copy(x1)."""
    x0 = list(_VALUES)
    x1 = list(_VALUES)
    x2 = list(_VALUES)
    return _fixture(
...
def m12_u6_g1_and_cone() -> HandcraftedFixture:
    """U6: Fabrizio G1; x2 := min(x0,x1), x3 := min(x1,x2)."""
    combos = list(itertools.product(_VALUES, _VALUES))
    x0 = [a for a, _ in combos]
    x1 = [b for _, b in combos]
    x2 = [min(a, b) for a, b in zip(x0, x1)]
    x3 = [min(b, c) for b, c in zip(x1, x2)]
    return _fixture(
...
def m12_u7_g1_or_cone() -> HandcraftedFixture:
    """U7: Fabrizio G1; x2 := max(x0,x1), x3 := max(x1,x2)."""
    combos = list(itertools.product(_VALUES, _VALUES))
    x0 = [a for a, _ in combos]
    x1 = [b for _, b in combos]
    x2 = [max(a, b) for a, b in zip(x0, x1)]
    x3 = [max(b, c) for b, c in zip(x1, x2)]
    return _fixture(
