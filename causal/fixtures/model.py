"""Validated model objects for exact positive finite-discrete causal BNs."""
...
@dataclass(frozen=True)
class CausalFixture:
    """A strictly positive finite-discrete causal Bayesian network.

    This model class deliberately describes the ordinary stochastic regime
    selected for the first M1.3 Bucket 3 fixture. Deterministic mechanisms need
    a separate model class and recovery assumptions later.
    """
    ...
    def validate(self) -> None:
        ...
        for variable in self.variables:
            ...
            for row in mechanism.rows:
                ...
                if any(probability <= 0 for probability in row.probabilities):
                    raise FixtureValidationError(
                        f"mechanisms.{variable.name}.cpt probabilities must all be strictly positive"
                    )
        ...
        if self.assumptions.causal_sufficiency_declared:
            expected_noise = "mutually_independent_private_exogenous_noise"
