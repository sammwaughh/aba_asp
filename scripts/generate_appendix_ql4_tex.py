"""Generate appendix-ql4.tex: QL1 + QL2 + QL4 tabular data (replaces QL3)."""
...
OUT = _REPO.parent / "appendix-ql4.tex"
...
QL4_ORDER = sorted(QI004_BUILDERS.keys())
...
def _meta_lines_ql4(key: str, fx) -> list[str]:
    base = key.rsplit("_", 1)[0]
    mode = key.rsplit("_", 1)[1]
    label, edges, parents = QL4_INFO[base]
    cfg_idx = list(QL4_INFO.keys()).index(base)
    seed = _QL4_BASE_SEED + cfg_idx * 10 + _MODES.index(mode)
    return [
...
def main() -> None:
    ...
    ql4_rows = sum(len(load_handcrafted(k).df) for k in QL4_ORDER)

    lines: list[str] = [
        "% appendix-ql4.tex — tabular input data for QL1, QL2, QL4",
        "% Requires in main.tex:",
        "%   \\usepackage{booktabs,longtable,geometry}",
        "% Include with: \\input{appendix-ql4}",
        "",
        "\\section{Tabular data for QL1, QL2, and QL4}",
        ...
        "without decimals. QL4 replaces the infeasible QL3 ($n=100$) design with the same",
        "structural configs at $n=20$.",
        "",
        "\\begin{table}[htbp]",
        "\\centering",
        ...
        f"QL4 & \\texttt{{QI004\\_scaled\\_motifs\\_n20}} & 15 & {ql4_rows} \\\\",
        "\\bottomrule",
        "\\end{tabular}",
        "\\end{table}",
        "",
    ...
    lines.extend(
        [
            "\\subsection{QL4 (QI-004): \\texttt{QI004\\_scaled\\_motifs\\_n20}}",
            "\\label{app:ql4}",
            ...
            f"(\\texttt{{\\_BASE\\_SEED = {_QL4_BASE_SEED}}}).",
            "",
        ]
    )

    for i, key in enumerate(QL4_ORDER, 1):
        fx = load_handcrafted(key)
        lines.append(f"\\subsubsection{{QL4 cell {i}/15: \\texttt{{{_tex_escape(key)}}}}}")
        lines.append(f"\\label{{app:ql4:{key.replace('_', '-')}}}")
        lines.extend(_meta_lines_ql4(key, fx))
        lines.append("")
        lines.extend(_longtable(key, fx, "ql4"))
